...
class BookingServiceImplTest {
    ...
    void shouldThrowExceptionWhenCreateIfDataNotRight(){
        when(bookMapper.requestDtoToBooking(any())).thenReturn(booking);
        NoEntityException ex = assertThrows(NoEntityException.class, ()->bookingService.create(1L,bookInDto));
        ...
        item.setOwner(user1);
        when(itemRepository.findById(anyLong())).thenReturn(Optional.of(item));
        AccessNotAllowed exAccept = assertThrows(AccessNotAllowed.class, ()->bookingService.create(1l,bookInDto));
    ...
    void shouldThrowExceptionWhenApproveWhenDataNotRight(){
        NoEntityException ex = assertThrows(NoEntityException.class,()->bookingService.approve(1L,1L,true));
        assertEquals("Booking is not found", ex.getMessage());

        when(bookRepository.findById(1L)).thenReturn(Optional.ofNullable(booking));
        AccessNotAllowed exAccess = assertThrows(AccessNotAllowed.class, ()->bookingService.approve(1L,1L,true));
        assertEquals("User id = 1 is not owner of booking item",exAccess.getMessage());

        item.setOwner(user1);
        booking.setStatus(BookingStatus.APPROVED);
        NotValidRequestException exRequest = assertThrows(NotValidRequestException.class,()->bookingService.approve(1L,1L,true));
    ...
    void shouldThrowExceptionWhenGetBookingIfDataNotRight(){
        NoEntityException ex = assertThrows(NoEntityException.class,()->bookingService.get(1L,1L));
        assertEquals("Booking is not found", ex.getMessage());

        booking.setBooker(user2);
        when(bookRepository.findById(1L)).thenReturn(Optional.of(booking));
        AccessNotAllowed exAccess = assertThrows(AccessNotAllowed.class, ()->bookingService.get(1L,1L));
    ...
    void shouldGetAllBookingsByBooker() {
        ...
        when(bookRepository.findAllByBookerIdOrderByStartDesc(1L,pageable)).thenReturn(List.of(bookingResult));
        List<BookingResponseDto> result = bookingService.getAllByBooker(1L,"ALL",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllByBookerIdAndStatus(1L,BookingStatus.WAITING, pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByBooker(1L,"WAITING",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllByBookerIdAndStatus(1L,BookingStatus.REJECTED, pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByBooker(1L,"REJECTED",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllCurrentByBookerId(1L, LocalDateTime.now(),pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByBooker(1L,"CURRENT",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllPastByBookerId(1L, LocalDateTime.now(),pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByBooker(1L,"PAST",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllFutureByBookerId(1L, LocalDateTime.now(),pageable)).thenReturn(List.of(bookingResult));
    ...
    void shouldGetAllBookingsByOwner() {
        ...
        when(bookRepository.findAllByItemOwnerIdOrderByStartDesc(1L,pageable)).thenReturn(List.of(bookingResult));
        List<BookingResponseDto> result = bookingService.getAllByOwner(1L,"ALL",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllByItemOwnerIdAndStatus(1L,BookingStatus.WAITING, pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByOwner(1L,"WAITING",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllByItemOwnerIdAndStatus(1L,BookingStatus.REJECTED, pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByOwner(1L,"REJECTED",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllCurrentByItemOwnerId(1L, LocalDateTime.now(),pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByOwner(1L,"CURRENT",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllPastByItemOwnerId(1L, LocalDateTime.now(),pageable)).thenReturn(List.of(bookingResult));
        result = bookingService.getAllByOwner(1L,"PAST",pageable);
        assertEquals(1,result.size());

        when(bookRepository.findAllFutureByItemOwnerId(1L, LocalDateTime.now(),pageable)).thenReturn(List.of(bookingResult));
    ...
    void shouldThrowExceptionWhenGetAllBookingsByBookerOrOwnerIfStateNotRight(){
        NotValidRequestException ex = assertThrows(NotValidRequestException.class, ()->bookingService.getAllByBooker(1L,"test",pageable));
        assertEquals("Unknown state: test",ex.getMessage());

        ex = assertThrows(NotValidRequestException.class, ()->bookingService.getAllByOwner(1L,"test",pageable));
        assertEquals("Unknown state: test",ex.getMessage());
    }
